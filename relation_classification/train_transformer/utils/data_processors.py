...
class MultiChoiceExample(object):
	...
	def __init__(self,
		...
		self.endings = [ending_0, ending_1, ending_2, ending_3, ending_4, ending_5, ]
		self.label = label
	...
	def __repr__(self):
		l = ["swag_id: {}".format(self.swag_id), "context_sentence: {}".format(self.context_sentence),
			 "ending_0: {}".format(self.endings[0]), "ending_1: {}".format(self.endings[1]),
			 "ending_2: {}".format(self.endings[2]), "ending_3: {}".format(self.endings[3]),
			 "ending_4: {}".format(self.endings[4]), "ending_5: {}".format(self.endings[5]), ]
	...
	def convert_examples_to_features(self,
		...
		for (ex_index, example) in enumerate(examples):
			...
			if forSequenceTagging:
				tokens_a, labels = tokenizer.tokenize_with_label_extension(example.text_a,
																		   example.labels,
				...
				if example.text_b:
					...
					if sep_token_extra:
						InputExample.truncate_seq_pair(tokens_a, tokens_b, max_seq_length - 4)
					else:
...
class ArgMinSeqTagProcessor(DataProcessor):
	...
	def __init__(self):
		...
		self.replace_labels = {'B-MajorClaim': 'B-Claim', 'I-MajorClaim': 'I-Claim', }
	...
	def get_train_examples(self, data_dir):
		"""See base class."""
		return self._create_examples(self._read_conll(os.path.join(data_dir, "train.conll"),
													  replace=self.replace_labels), "train")

	def get_dev_examples(self, data_dir):
		"""See base class."""
		return self._create_examples(self._read_conll(os.path.join(data_dir, "dev.conll"),
													  replace=self.replace_labels),
									 "dev")

	def get_test_examples(self, data_dir, setname="test.conll"):
		"""See base class."""
		return self._create_examples(self._read_conll(os.path.join(data_dir, setname), replace=self.replace_labels),
									 "test")
	...
	def _create_examples(self, lines, set_type):
		...
		for (i, line) in enumerate(lines):
			...
			text_a = line[0]
			labels = line[-1]
			examples.append(InputExample(guid=guid, text_a=text_a, text_b=None, labels=labels))
		return examples
...
class ArgMinRelClassProcessor(DataProcessor):
	...
	def __init__(self):
		self.labels = ["__label__noRel", "__label__Support", "__label__Attack"]
		self.label_map = self._create_label_map()
	...
	def get_train_examples(self, data_dir):
		"""See base class."""
		return self._create_examples(self._read_tsv(os.path.join(data_dir, "train_relations.tsv")), "train")

	def get_dev_examples(self, data_dir):
		"""See base class."""
		return self._create_examples(self._read_tsv(os.path.join(data_dir, "dev_relations.tsv")), "dev")

	def get_test_examples(self, data_dir, setname="test_relations.tsv"):
		"""See base class."""
		return self._create_examples(self._read_tsv(os.path.join(data_dir, setname)), "test")
	...
	def _create_examples(self, lines, set_type):
		...
		for (i, line) in enumerate(lines):
			...
			text_b = line[2]
			label = line[0]
			examples.append(InputExample(guid=guid, text_a=text_a, text_b=text_b, labels=label))
		return examples
...
class ArgMinMultiChoiceLinkProcessor(DataProcessor):
	...
	def __init__(self):
		super().__init__()
		self.labelmap = {"NoRelation": 2, "Support": 0, "Attack": 1, "Partial-Attack": 1}

	def get_train_examples(self, data_dir):
		"""See base class."""
		return self._create_examples(self._read_tsv(os.path.join(data_dir, "train_mc.tsv")), "train")

	def get_dev_examples(self, data_dir):
		"""See base class."""
		return self._create_examples(self._read_tsv(os.path.join(data_dir, "dev_mc.tsv")), "dev")
	...
	def _create_examples(self, lines, set_type):
		...
		for i, line in enumerate(lines):
			...
			examples.append(MultiChoiceExample(swag_id=guid,
	...
	def convert_examples_to_features(self, examples, tokenizer, max_seq_length, logger=None):
		...
		# The model will data_relclass a single value for each input. To get the
		...
		for example_index, example in enumerate(examples):
			...
			features.append(MultiChoiceInputFeatures(example_id=example.swag_id,
...
class OutcomeClassificationProcessor(DataProcessor):
	"""Processor for the RCT outcome data """
	...
	def get_test_examples(self, data_dir, setname="test.tsv"):
		"""See base class."""
		return self._create_examples(self._read_tsv(os.path.join(data_dir, setname)), "test")

	def get_labels(self):
		"""See base class."""
		return ["Improved", "Increased", "Decreased", "NoDifference", "NoOccurrence"]

	def _create_examples(self, lines, set_type):
		"""Creates examples for the training and dev sets."""
		examples = []
		relevant_labels = self.get_labels()
		for (i, line) in enumerate(lines):
			# skip first line (e.g. PE dataset)
			# if i == 0:
			#    continue
...
processors = {
	...
	"outcomeclf" : OutcomeClassificationProcessor,
