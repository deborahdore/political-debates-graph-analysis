...
class ExtendedBertTokenizer(BertTokenizer):
	...
	def tokenize_with_label_extension(self, text, labels, copy_previous_label=False, extension_label='X'):
		...
		:param copy_previous_label: boolean if previous label gets copied as new sub-token or if extension_label is
		used
	...
	def batch_to_conll(self, input_ids, label_ids, gold_label=None, processor=None, output_file=None):
		...
		for i, _ in enumerate(input_ids):  # iterate over batch
			...
			for j, token_id in enumerate(input_ids[i].cpu().numpy()):  # iterate over tokens of one sequence
				...
				token = " ".join(token).replace(' ##', '').replace('Ġ', '')
...
class ExtendedRobertaTokenizer(RobertaTokenizer):
	"""BertTokenizer which extends a token-based list of labels to WordPiece sub-token-based list.
	Params, Inputs, Outputs:
		See base class.
	"""
	...
	def batch_to_txt(self, input_ids, label_ids, gold_label=None, processor=None, output_file=None):
		"""
