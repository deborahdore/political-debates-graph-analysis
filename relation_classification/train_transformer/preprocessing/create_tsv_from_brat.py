			...
			for c_i in components.keys():
				for c_j in components.keys():
					if c_i == c_j:
						...
						useful_pairs.append('__label__' + relations[c_i] + '\t' + components[c_i] + '\t' + components[
							c_j])
					else:

						if (
								c_i in list(links.values()) or c_j in links.keys() or c_j in list(links.values()) or
								c_i in links.keys()):
...
def main():
	...
	create_tsv_for_relation_classification(input_dir, output_file)  # create_tsv_for_multiple_choice(input_dir, output)
