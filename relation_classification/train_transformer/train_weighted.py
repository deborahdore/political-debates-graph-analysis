...
from train_transformer.utils.data_processors import output_modes, processors
from train_transformer.utils.metrics import compute_metrics
from train_transformer.utils.models import BertForSequenceClassificationWeighted, \
	BertForSequenceTagging, \
	RobertaForSequenceClassificationWeighted
from train_transformer.utils.tokenizer import ExtendedBertTokenizer, ExtendedRobertaTokenizer
...
def train(args, train_dataset, model, tokenizer):
	...
	class_count = [18570, 15931, 2383]
	weights = 1 / torch.Tensor(class_count)
	samples_weights = np.array([weights[int(t[-1])] for t in train_dataset])
	samples_weights = torch.from_numpy(samples_weights)

	# train_sampler = WeightedRandomSampler(samples_weights, 2*len(train_dataset))
		...
		for step, batch in enumerate(epoch_iterator):
			...
			if args.model_type != "distilbert":
				inputs["token_type_ids"] = (
					batch[2] if args.model_type in ["bert", "xlnet", "albert"] else None)  # XLM, DistilBERT,
			# RoBERTa, and XLM-RoBERTa don't use segment_ids
			...
			if (step + 1) % args.gradient_accumulation_steps == 0:
				...
				if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
					logs = {}
					if (args.local_rank == -1 and args.evaluate_during_training):  # Only evaluate when single GPU
				...
				if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
					...
					model_to_save = (model.module if hasattr(model, "module") else model)  # Take care of
...
def evaluate(args, model, tokenizer, prefix=""):
	...
	for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
		...
		# data_relclass file to write predictions
		if args.task_name == "seqtag":
			suffix = '.conll'
		else:
			suffix = '.tsv'
		output_file = os.path.join(os.path.join(eval_output_dir, prefix), "predictions" + suffix)
		...
		for batch in tqdm(eval_dataloader, desc="Evaluating"):
			...
			with torch.no_grad():
				...
				if args.model_type != "distilbert":
					inputs["token_type_ids"] = (
						batch[2] if args.model_type in ["bert", "xlnet", "albert"] else None)  # XLM,
				# DistilBERT, RoBERTa, and XLM-RoBERTa don't use segment_ids
				...
				if args.output_mode == "sequencetagging":
					...
					# Write data_relclass as conll file
					...
					tokenizer.batch_to_conll(inputs["input_ids"], logits, labels, processor, "new" + output_file)
		...
		if args.task_name != "seqtag":
			with open(output_file, "a") as writer:
				for i, _ in enumerate(preds):
					writer.write(str(out_label_ids[i]) + '\t' + str(preds[i]) + '\n')
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
	...
	cached_features_file = os.path.join(args.data_dir,
										"cached_{}_{}_{}_{}".format("dev" if evaluate else "train",
																	list(filter(None,
	...
	if os.path.exists(cached_features_file) and not args.overwrite_cache:
		...
		examples = (
			processor.get_dev_examples(args.data_dir) if evaluate and args.do_train else processor.get_test_examples(
		...
		features = processor.convert_examples_to_features(examples,
														  ...
														  tokenizer=tokenizer,
														  logger=logger,
														  forSequenceTagging=forSequenceTagging,
														  min_seq_length=5,
...
def main():
	...
	parser.add_argument("--output_dir",
						...
						type=str,
						required=True,
						help="The data_relclass directory where the model predictions and checkpoints will be "
							 "written.", )
	...
	parser.add_argument("--overwrite_output_dir",
						action="store_true",
						help="Overwrite the content of the data_relclass directory", )
	...
	if (
			os.path.exists(args.output_dir) and os.listdir(args.output_dir) and args.do_train and not
	...
	if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
		# Create data_relclass directory if needed
