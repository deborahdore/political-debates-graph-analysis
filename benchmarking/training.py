...
def training(
		model_dir: str, model_name: str, results_dir: str, plot_dir: str, train_factory: TriplesFactory,
		test_factory: TriplesFactory, val_factory: TriplesFactory, ratio: float
		) -> None:
	"""
	...
	result = pipeline(
			training = train_factory, testing = test_factory, validation = val_factory,
			model = model_name,
...
def hyperparameter_optimization(
		model_name: str, model_dir: str, train_factory: TriplesFactory, test_factory: TriplesFactory,
		val_factory: TriplesFactory, ratio: float
		) -> None:
	"""
	...
	hpo_results = hpo_pipeline(
			...
			training_kwargs_ranges = dict(
					num_epochs = dict(type = int, low = 30, high = 200, q = 5),
